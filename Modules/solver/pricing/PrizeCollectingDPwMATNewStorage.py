...
# refactoring the previous prizeCollectingDPVer2
class PrizeCollectingDPwMATNewStorage(LabelSetting):
    def __init__(self, _n: int, _C: Dict, _Q: List, _dual: np.ndarray,
        ...
        self.Q = _Q
        self.dual = _dual[:-1]
        self.mu = _dual[-1] # last dual coefficient is for the fleet size constraint
        self.s0 = _s0
    ...
    def solve(self) -> Tuple[Dict[int, List[LabelMATModel]], Tuple[int, int]]:        
        ...
        initial_label = LabelMATModel(0, 0, 0, 0, 0, 0, 0, False, False, 0, False)
    ...
    # refactored from transRwdVer2
    def get_transition_reward(self, current_state: LabelMATModel,
                              _j,_opt_m):
        _i = current_state.i
        _nD = current_state.acc_demand + self.Q[_j]
        _nPi = self.dual[_j-1] #dual's index doesnt have depot
        _nM = _opt_m
        _mu = self.mu
        _reCostI = self.C[(_i,0)]
        _reCostJ = self.C[(_j,0)]
        if (current_state.m==0): # extend from start state
            _tRwd = _nPi +_mu*(_nM) \
                    - (self.Q[_j]*(current_state.acc_length+self.C[(_i,_j)])) \
                    - ((0.5/_nM)*_nD*(current_state.acc_length+self.C[(_i,_j)]+_reCostJ+self.s0))
        else:
            _tRwd = _nPi +_mu*(_nM-current_state.m)\
                    - (self.Q[_j]*(current_state.acc_length+self.C[(_i,_j)])) \
                    - ((0.5/_nM)*_nD*(current_state.acc_length+self.C[(_i,_j)]+_reCostJ+self.s0))\
                    + ((0.5/current_state.m)*current_state.acc_demand*(current_state.acc_length+_reCostI+self.s0))
        return _tRwd
    
    def get_optimal_m_for_state(self, M, q, lr, mu):
        phi = self.phi(q, lr, mu)
        cphi = np.ceil(phi)
        fphi = np.floor(phi)
        cel_h = self.h_func(cphi, q, lr, mu)
        flr_h = self.h_func(fphi, q, lr, mu)
        if (cel_h >= flr_h) and (cphi <= M):
            opt_m = cphi
        elif (cel_h < flr_h) and (fphi <= M):
            opt_m = fphi
        else:
            opt_m = M
        return opt_m

    def phi(self, d, lr, mu):
        return np.sqrt((d)*(lr)/(2*np.abs(mu)))

    def h_func(self, phi, q, lr, mu):
        if phi > 0:
            return (mu * (phi)) - ((q * lr) / (2 * phi))
        else:
            return 0
    ...
    def convert_to_legacy_format(self, S: Dict[int, List[LabelMATModel]]) -> List[List[List[Any]]]:
    ...
    def _check_dominance(self, new_label: LabelMATModel) -> bool:
        """
    ...
    def _check_dominance_ver4(self, state_a: LabelMATModel, state_b: LabelMATModel) -> bool:
        if ( (state_a.acc_demand <= state_b.acc_demand) and
            (state_a.acc_length <= state_b.acc_length) and
            (state_a.reward - state_a.h_opt >= state_b.reward - state_b.h_opt)):
            return True
        return False
    
    def _filter_out_dominated_states(self, new_label: LabelMATModel)-> bool:
        """
    ...
    def get_dominant_type(self, label: LabelMATModel):
        ...
        if label.acc_length + self.C[(label.i, label.prevN)] > self.time_window:
            ...
            # opt m for next state exceeds total fleet size
            next_d = label.acc_demand + self.Q[label.prevN]
            next_l = label.acc_length + self.C[(label.i, label.prevN)]
            m_ctc = np.ceil((next_d * (next_l + self.C[(label.prevN, 0)])) / self.veh_cap)
            m_tw = np.ceil((next_l + self.C[(label.prevN, 0)]) / (self.time_window - next_l))
            next_m_opt = self.get_optimal_m_for_state(self.total_fleet_size, next_d, next_l+self.C[(label.prevN,0)], self.mu)
            next_m_opt_eff = next_m_opt if next_m_opt >= max(m_ctc, m_tw) else max(m_ctc, m_tw)
            if next_m_opt_eff > self.total_fleet_size:
                label.dominance_label = DominanceLabel.STRONGLY_DOMINANT
            else:
                label.dominance_label = DominanceLabel.SEMISTRONGLY_DOMINANT

    def get_label_priority(self, label: LabelMATModel) -> Tuple[float, float, float]:
        """
